...
public:
    ...
    bool Insert(string name, string baseType, bool isArray, int arraySize, int line)
    {
        if (currentTable == NULL)
            return false;
    ...
    bool Insert(string name, string type, bool isFunction, string returnType, const vector<string> &paramTypes, int line)
    {
        if (currentTable == NULL)
            return false;
    ...
    void ExitAllScope()
    {
        while (currentTable != NULL && currentTable->parent_scope != NULL)
        { // Stop before global scope
            ExitScope();
        }
    }

    // Method to get all global variables for assembly generation
    vector<SymbolInfo *> getAllGlobalVariables()
    {
        vector<SymbolInfo *> globalVars;

        // Get the global scope (root scope)
        ScopeTable *globalScope = currentTable;
        while (globalScope != nullptr && globalScope->parent_scope != nullptr)
        {
            globalScope = globalScope->parent_scope;
        }

        if (globalScope != nullptr)
        {
            // Traverse the hash table in global scope
            for (int i = 0; i < globalScope->getNumBuckets(); i++)
            {
                SymbolInfo *current = globalScope->getBucketHead(i);
                while (current != nullptr)
                {
                    // Only include variables and arrays, not functions
                    if (!current->getIsFunction())
                    {
                        globalVars.push_back(current);
                    }
    ...
    // Method to check if a variable is in global scope
    bool isVariableGlobal(const string &varName)
    {
        // Get the global scope (root scope)
        ScopeTable *globalScope = currentTable;
        while (globalScope != nullptr && globalScope->parent_scope != nullptr)
        ...
        if (globalScope != nullptr)
        {
            return globalScope->LookUp(varName) != nullptr;
        }
